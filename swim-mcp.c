...
// --- Table-driven dispatch for simple methods ---

typedef enum { M_GET, M_POST_STRING, M_POST_FIXED } MethodKind;
...
typedef struct {
    ...
    MethodKind kind;
    const char *param;
    const char *fixed_body;
} SimpleMethod;

static const SimpleMethod simple_methods[] = {
    {"extract",          "/extract",  M_GET,         NULL,       NULL},
    {"interact",         "/interact", M_GET,         NULL,       NULL},
    {"console",          "/console",  M_GET,         NULL,       NULL},
    {"dialog",           "/dialog",   M_GET,         NULL,       NULL},
    {"requests",         "/requests", M_GET,         NULL,       NULL},
    {"state",            "/state",    M_GET,         NULL,       NULL},
    {"scroll",           "/scroll",   M_POST_STRING, "selector", NULL},
    {"hover",            "/hover",    M_POST_STRING, "selector", NULL},
    {"key",              "/key",      M_POST_STRING, "key",      NULL},
    {"eval",             "/eval",     M_POST_STRING, "js",       NULL},
    {"execute",          "/command",  M_POST_STRING, "command",  NULL},
    {"navigate_back",    "/action",   M_POST_FIXED,  NULL, "{\"action\":\"back\"}"},
    {"navigate_forward", "/action",   M_POST_FIXED,  NULL, "{\"action\":\"forward\"}"},
    {NULL, NULL, 0, NULL, NULL}
};

static char *handle_tool_call(const char *name, const char *arguments) {
    // Table-driven dispatch for simple methods
    for (int i = 0; simple_methods[i].method; i++) {
        if (strcmp(name, simple_methods[i].method) != 0) continue;
        const SimpleMethod *m = &simple_methods[i];
        switch (m->kind) {
        case M_GET:
            return get_passthrough(m->endpoint);
        case M_POST_STRING:
            return post_string_param(m->endpoint, m->param, arguments);
        case M_POST_FIXED: {
            char *resp = http_post(m->endpoint, m->fixed_body);
            return resp ? resp : strdup("{\"error\":\"connection failed\"}");
        }
        }
    }
...
static char *read_mcp_message(void) {
    // Read newline-delimited JSON with growable buffer
    int cap = 65536;
    char *buf = malloc(cap);
    if (!buf) return NULL;
    int len = 0;

    // Read byte-by-byte until newline, growing buffer as needed
    for (;;) {
        int c = read_byte();
        if (c < 0) { if (len == 0) { free(buf); return NULL; } break; }
        if (len >= cap - 1) {
            int new_cap = cap * 2;
