...
static NSDictionary *do_requests(ServeContext *ctx) {
    NSString *js =
        @"(function(){"
        "var BODY_MAX=4096;"
        "function clampBody(b){if(!b)return undefined;var s=String(b);return s.length>BODY_MAX?s.substring(0,BODY_MAX)+'...(truncated)':s}"
        ...
        "    var raw=h.getAllResponseHeaders()||'';raw.split('\\r\\n').forEach(function(l){"
        "      var i=l.indexOf(':');if(i>0)o[l.substring(0,i).trim().toLowerCase()]=l.substring(i+1).trim()"
        "    })"
        "  }"
        "  return o}"
        ...
        "    if(opts.headers){var rh={};var h=opts.headers;"
