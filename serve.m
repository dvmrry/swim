...
static bool translate_key(const char *name, const char **out_key, unsigned int *out_mods) {
    ...
    // Ctrl-X combos: compute from letter
    if (strncmp(name, "Ctrl-", 5) == 0 && name[5] >= 'A' && name[5] <= 'Z' && name[6] == '\0') {
        static char ctrl_buf[2];
        ctrl_buf[0] = (char)(name[5] - 'A' + 1);
        ctrl_buf[1] = '\0';
        *out_key = ctrl_buf;
        *out_mods = MOD_CTRL;
        return true;
...
static NSDictionary *do_state(ServeContext *ctx) {
    __block NSDictionary *result = nil;
    dispatch_sync(dispatch_get_main_queue(), ^{
        const char *mode_str = mode_name(ctx->mode->mode);
...
static NSDictionary *do_console(ServeContext *ctx) {
    // Hooks injected at DocumentStart via WKUserScript; just drain the buffer
    return eval_js_sync(ctx,
        @"(function(){"
...
// --- Route table ---
// Two handler signatures: GET (no json) and POST (with json body).
// Screenshot, PDF, health have custom handlers; everything else uses do_* dispatch.

typedef NSDictionary *(*do_get_fn)(ServeContext *ctx);
typedef NSDictionary *(*do_post_fn)(NSDictionary *json, ServeContext *ctx);

typedef struct {
    const char *path;
    bool is_post;
    union { do_get_fn get; do_post_fn post; } fn;
} Route;

static const Route routes[] = {
    {"/state",    false, {.get  = do_state}},
    {"/extract",  false, {.get  = do_extract}},
    {"/interact", false, {.get  = do_interact}},
    {"/console",  false, {.get  = do_console}},
    {"/dialog",   false, {.get  = do_dialog}},
    {"/requests", false, {.get  = do_requests}},
    {"/action",   true,  {.post = do_action}},
    {"/command",  true,  {.post = do_command}},
    {"/key",      true,  {.post = do_key}},
    {"/resize",   true,  {.post = do_resize}},
    {"/wait",     true,  {.post = do_wait}},
    {"/wait_for", true,  {.post = do_wait_for}},
    {"/eval",     true,  {.post = do_eval}},
    {"/fill",     true,  {.post = do_fill}},
    {"/click",    true,  {.post = do_click}},
    {"/query",    true,  {.post = do_query}},
    {"/tab",      true,  {.post = do_tab}},
    {"/select",   true,  {.post = do_select}},
    {"/scroll",   true,  {.post = do_scroll}},
    {"/storage",  true,  {.post = do_storage}},
    {"/hover",    true,  {.post = do_hover}},
    {"/drag",     true,  {.post = do_drag}},
    {"/upload",   true,  {.post = do_upload}},
};

static const int route_count = sizeof(routes) / sizeof(routes[0]);

// Dispatch a named operation (used by both route handler and /batch)
static NSDictionary *dispatch_op(const char *name, NSDictionary *json, ServeContext *ctx) {
    for (int i = 0; i < route_count; i++) {
        if (strcmp(name, routes[i].path + 1) == 0) {  // skip leading '/'
            return routes[i].is_post ? routes[i].fn.post(json, ctx)
                                     : routes[i].fn.get(ctx);
        }
    }
    // Non-table operations
    if (strcmp(name, "screenshot") == 0) return do_screenshot(ctx);
    if (strcmp(name, "sleep") == 0) return do_sleep_step(json);
    return nil;
}
...
static void handle_batch(int fd, HTTPRequest *req, ServeContext *ctx) {
    ...
    for (NSDictionary *step in steps) {
        ...
        NSDictionary *result = dispatch_op([type UTF8String], step, ctx);
        [results addObject:result ?: @{@"ok": @NO, @"error":
            [NSString stringWithFormat:@"unknown type: %@", type]}];
    }

    send_dict(fd, @{@"ok": @YES, @"results": results});
}
...
static void *server_thread(void *arg) {
    ...
    while (1) {
        ...
        // Special routes with custom response formats
        if (strcmp(req.path, "/health") == 0) {
            send_json(client_fd, 200, "{\"ok\":true}");
            handled = true;
        } else if (strcmp(req.path, "/screenshot") == 0) {
            handle_screenshot(client_fd, ctx);
            handled = true;
        } else if (strcmp(req.path, "/pdf") == 0) {
            handle_pdf(client_fd, ctx);
            handled = true;
        } else if (strcmp(req.path, "/batch") == 0) {
            handle_batch(client_fd, &req, ctx);
            handled = true;
        }

        // Table-driven routes: parse JSON body → do_* → send_dict
        if (!handled) {
            for (int i = 0; i < route_count; i++) {
                if (strcmp(req.path, routes[i].path) == 0) {
                    ...
                    if (routes[i].is_post) {
                        ...
                        result = routes[i].fn.post(json, ctx);
                    } else {
                        result = routes[i].fn.get(ctx);
                    }
                    send_dict(client_fd, result);
                    handled = true;
                    break;
                }
            }
