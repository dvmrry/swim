...
### App Mode (Electron Replacement)

`swim --app <url|dir>` — single WKWebView window, no vim keys, no tab
bar, no status bar. Reads an optional `manifest.json` for window
size/title/icon. Bridge API injected via `WKScriptMessageHandler`
exposes `window.swim.*` (screenshot, file I/O, native dialogs).

~200KB binary vs Electron's ~150MB. Uses system WebKit (free security
updates, shared memory). Already has the HTTP API for external
automation/testing. The missing pieces are small: a `--app` flag that
skips mode_init and hides chrome, plus a JS bridge userscript.

**Status:** Idea. Not personally high-value yet. Becomes near-free if
libswim extraction happens first.
...
- Web-based browser chrome (swim's own UI stays native Cocoa)
